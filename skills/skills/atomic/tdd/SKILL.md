...
- **The interface itself isn't clear.** Stop. Run [`grill-me`](../grill-me/SKILL.md) on the interface design first.
- **You can't build a fast feedback loop.** Run [`diagnose`](../diagnose/SKILL.md) phase 1 to build one before resuming TDD.
- **Behaviour spans modules you don't know.** Run [`zoom-out`](../zoom-out/SKILL.md) to orient before slicing.
