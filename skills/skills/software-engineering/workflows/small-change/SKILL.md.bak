...
Hand off to [`diagnose`](../../atomic/diagnose/SKILL.md) for the full feedback-loop discipline if the bug fights back.
...
**Then ask once:** *what would have prevented this bug?* If the answer is architectural (no good test seam, tangled callers, hidden coupling), file it for [`improve-architecture`](../../atomic/improve-architecture/SKILL.md). Do not refactor mid-fix.

## When to escalate

- The "small change" reveals a schema migration, public API change, or multi-module ripple → re-route to **`medium-feature`**.
- The bug needs domain-language clarification before it can even be defined → run [`grill-me`](../../atomic/grill-me/SKILL.md) first.
- The fix exposes a structural problem → finish the fix narrowly, then hand off to [`improve-architecture`](../../atomic/improve-architecture/SKILL.md).
