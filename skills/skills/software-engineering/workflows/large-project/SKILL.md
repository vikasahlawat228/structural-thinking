...
Before any spec, before any code, the agent needs to speak your project's language. Run [`grill-with-docs`](../../../general/grill-with-docs/SKILL.md) at the highest level: what is this project, in your team's vocabulary?
...
Run [`to-prd`](../../to-prd/SKILL.md) for the full project. A large PRD is multi-page; that's fine — but each section must still be load-bearing.
...
Software entropy accelerates with agents. The defence is **regular, deliberate deepening passes**. Once a week — or on a tangible signal like "agents are getting confused" — run [`improve-architecture`](../../improve-architecture/SKILL.md) on the codebase.
