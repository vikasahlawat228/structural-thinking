...
Run [`grill-me`](../../atomic/grill-me/SKILL.md). Goal: surface the decision tree before you touch code.
...
Run [`to-prd`](../../atomic/to-prd/SKILL.md). For a medium feature, the PRD is short — one page, max.
...
Run [`to-issues`](../../atomic/to-issues/SKILL.md). Break the feature into **vertical slices** — each slice must be:
...
For each slice, run [`tdd`](../../atomic/tdd/SKILL.md):
...
Then run [`zoom-out`](../../atomic/zoom-out/SKILL.md) once: how does this feature sit in the broader system? Anything to flag for `improve-architecture` later?
...
- The grill keeps producing new branches → escalate to **`large-project`**, run [`grill-with-docs`](../../atomic/grill-with-docs/SKILL.md) instead.
...
- If you're swapping agents or the session is running long, run [`handoff`](../../atomic/handoff/SKILL.md) before continuing.
